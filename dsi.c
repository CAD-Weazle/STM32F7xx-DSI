...
// last    : 251226
//
// note    : to enable the tearing effect signal coming from the TFT, the command 
//           'OTM8009A_CMD_TEEON' with argument '0x00' must be sent during initialization
//           of the displu - this is not in the ST example
//

#include "includes.h"

// image data ready in SDRAM
int new_image = 0;

// handle Tearing Effect signal - from OTM8009A on TFT add-on board
void DSI_IRQHandler (void)
{
  // check Tearing Effect interrupt
  if (DSI->WISR & DSI_WISR_TEIF)
  {
    // clear interrupt 
    DSI->WIFCR |= DSI_WIFCR_CTEIF;

    // update TFT display on new image
    if (new_image != 0)
    {
      // send image data & clear flag
      dsi_refresh ();      
      new_image = 0;
    }
  }
...
void dsi_adapted_config (void)
{
  ...
  DSI->WCFGR = (0b101 << DSI_WCFGR_COLMUX_Pos) // 24-bit color multiplexing
           ...
           //| DSI_WCFGR_TEPOL                 // 0: rising / 1: falling edge
             | DSI_WCFGR_TESRC;                // tearing effect via external pin
  ...
  // enable Tearing Effect Request - tearing effect via DSI link, not used
...
void dsi_host_config (void)
{
  ...
  // DSI interrupt for handling tearing effect interrupt
  NVIC_SetPriority (DSI_IRQn, 3);
  NVIC_EnableIRQ (DSI_IRQn);
}

// setup Layer 1, image data in SDRAM
void dsi_layer1_config (void)
{
  // set horizontal start and stop position
  LTDC_Layer1->WHPCR = ((HACT + HSYNC + HBP) << 16) | (HSYNC + HBP); // full width when using SDRAM 
//LTDC_Layer1->WHPCR = ((HACT-XBORDER) << 16) | XBORDER;             // small area to fit SRAM

  // set vertical start and stop position
  LTDC_Layer1->WVPCR = ((VACT + VSYNC + VBP) << 16) | (VSYNC + VBP); // full height when using SDRAM (VACT+2 ???)
...
// test data path - color a rectangle - dev only
void fill_rect (void)
{
...
//printf ("filling color: 0x%06lx\n", color);
  ...
  // signal new image data ready
  new_image = 1;

  // update image memory to display
//dsi_refresh (); // in ISR now
}

// simple color changer - dev only
#define RED_UP    0
#define RED_DN    1
#define GRN_UP    2
#define GRN_DN    3
...
{
  static int state = RED_UP;
  
  static uint32_t red = VAL_MIN;
  static uint32_t grn = VAL_MIN;
  static uint32_t blu = VAL_MAX;
